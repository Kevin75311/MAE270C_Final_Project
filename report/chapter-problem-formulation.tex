...
\begin{figure}[htbp]
    ...
    \caption{Domain geometry and sub-regions: tumor $\Omega_T$ (red), safety margin $\Omega_M$ (yellow), and healthy tissue $\Omega_H$ (green).}
    \label{fig:ablation}
\end{figure}
...
the Pennes bioheat equation~\cite{pennes1948analysis}:
\begin{equation}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/test_bioheat_2d.png}
    \caption{Verification of the \texttt{BioHeatSolver} under constant
             $P = \SI{50}{\watt}$: the temperature field grows around the
             probe (top), and the tumor-center temperature rises while the
             healthy-tissue peak stays below $T_{\text{safe}}$ (bottom),
             confirming the discretized Pennes
             dynamics~\eqref{eq:bioheat-discrete}.}
    \label{fig:test-bioheat}
\end{figure}
...
of irreversible cell necrosis (Henriques--Moritz
criterion~\cite{moritz1947studies}).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/test_arrhenius_2d.png}
    \caption{Verification of the \texttt{ArrheniusDamage} integrator at a
             constant $T = \SI{60}{\degreeCelsius}$. The damage field
             crosses the necrosis threshold $\Omega_d = 1$ at $t \approx
             \SI{1.0}{\second}$ (right), matching the analytical crossing
             time of $\SI{0.9}{\second}$ to within one time step.}
    \label{fig:test-arrhenius}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/test_bc_2d.png}
    \caption{Effect of the three boundary conditions on a uniform
             $\SI{60}{\degreeCelsius}$ field. Dirichlet pins the boundary
             ring to $T_{\mathrm{wall}} = \SI{37}{\degreeCelsius}$ (red
             border); Neumann leaves the interior unchanged (zero flux);
             Robin cools the boundary toward $T_\infty = \SI{20}{\degreeCelsius}$
             at rate $h_c$.}
    \label{fig:test-bc}
\end{figure}
...
Symbol & Role & Default \\
\midrule
$\alpha_1$ & Energy penalty ($\|P\|^2$) & $5\times10^{-4}$ \\
$\alpha_2$ & Healthy-tissue overheating & $1.0$ \\
$\alpha_3$ & Time-rate cost (encourages speed) & $0.01$ \\
$\gamma_1$ & Incomplete ablation penalty (area-integrated) & $10^{6}$ \\
$\gamma_2$ & Soft minimum-time penalty & $0.1$ \\
\bottomrule
\end{tabular}
\end{table}

The ablation penalty is a \emph{spatial integral} scaled by the voxel
area $dA \approx \SI{e-6}{\square\meter}$, so $\gamma_1$ must be large
($10^6$) for the ``kill the tumor'' objective to dominate the
$O(10^1$--$10^2)$ energy and time costs; a smaller $\gamma_1$ makes
``do nothing'' the global optimum. This is the \emph{non-aggressive}
weight set used for every run in Chapter~\ref{ch:results}.

%----------------------------------------------------------------------
\section{Adjoint Equations and Optimal Control Law}

Applying the \gls{pmp}~\cite{pontryagin2018mathematical} to the
discretized system, the costate vector
