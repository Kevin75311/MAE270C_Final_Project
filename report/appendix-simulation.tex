...
Run each solver mode, optionally selecting dimensionality and probe:
\begin{verbatim}
...
python main.py --mode indirect --ndim 3         # 3-D volume
python main.py --mode openloop --probe dipole   # dipole SAR geometry
...
\texttt{DomainConfig.Nx/Ny/Nz}        & $N_x, N_y, N_z$ & 50, 50, 50  & --- \\
...
\texttt{ControlConfig.T\_safe}        & $T_{\text{safe}}$ & 50.0      & \si{\degreeCelsius} \\
...
\texttt{CostConfig.alpha1}            & $\alpha_1$      & $5\times10^{-4}$ & --- \\
\texttt{CostConfig.gamma1}            & $\gamma_1$      & $10^{6}$    & --- \\
\texttt{CostConfig.gamma2}            & $\gamma_2$      & 0.1         & --- \\
\bottomrule
\end{tabular}
\end{table}
