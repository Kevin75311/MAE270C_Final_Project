...
\end{enumerate}
Convergence is sensitive to $\beta$ (default 0.5);
oscillation indicates $\beta$ should be reduced.
...
\subsection{Mode 3 — Receding-Horizon MPC}

At each control update interval $\Delta t_{\text{ctrl}}$, a short-horizon
optimization is solved using the current (possibly noisy) MRI temperature
measurement as the initial condition.
A bang-off-bang heuristic is used for the inner optimizer in the current
implementation; this can be replaced by \texttt{solve\_direct()} or
\texttt{solve\_indirect()} on the horizon window for a true MPC.

MRI measurement noise is modeled as additive Gaussian:
$\hat{T} = T_{\text{true}} + \mathcal{N}(0,\, \sigma_{\text{MRI}}^2)$
with $\sigma_{\text{MRI}} = \SI{0.5}{\degreeCelsius}$.
...
Control  & \texttt{control/solver.py}            & Four solver backends \\
